...
export default async function CandidateDashboardContent({ userId }: { userId: string }) {
  ...
  return (
    <>
      ...
      {/* Mobile-Only Horizontal Coworker Avatar Reel (< 1024px) */}
      <div className="lg:hidden flex flex-col gap-1.5 mt-2.5">
        <div className="flex items-center justify-between px-0.5">
          <span className="text-[11px] font-bold uppercase tracking-wider text-[var(--md-sys-color-on-surface-variant)] flex items-center gap-1.5">
            <Users className="w-3.5 h-3.5 text-emerald-500" />
            Working Right Now ({workingNowCount})
          </span>
        ...
        {allActiveSessions && allActiveSessions.length > 0 ? (
          <div className="flex items-center gap-2 overflow-x-auto py-1 no-scrollbar">
            {allActiveSessions.map((session) => {
              ...
              const firstName = profileObj?.full_name?.split(' ')[0] || 'Member'

              return (
                <div
                  key={session.id}
                  className={`flex items-center gap-1.5 pl-1.5 pr-2.5 py-1 rounded-full border text-xs shrink-0 active:scale-95 transition-all ${
                    isMe
                      ? 'bg-[var(--md-sys-color-primary-container)] border-[var(--md-sys-color-primary)] text-[var(--md-sys-color-on-primary-container)] font-semibold'
                      : isOnBreak
                      ? 'bg-amber-500/10 border-amber-500/30 text-amber-800 dark:text-amber-300'
                      : 'bg-[var(--md-sys-color-surface)] border-[var(--md-sys-color-outline-variant)] text-[var(--md-sys-color-on-surface)]'
                  }`}
                >
                  <div className="relative w-5 h-5 rounded-full bg-[var(--md-sys-color-surface-container-highest)] flex items-center justify-center font-bold text-[10px]">
                    {firstName.charAt(0).toUpperCase()}
                    <span
                      className={`absolute -bottom-0.5 -right-0.5 w-2 h-2 rounded-full border border-[var(--md-sys-color-surface)] ${
                        isOnBreak ? 'bg-amber-500' : 'bg-emerald-500'
                      }`}
                    />
                  </div>
                  <span className="text-[11px] truncate max-w-[80px]">{isMe ? 'You' : firstName}</span>
                </div>
              )
            })}
          </div>
        ) : (
          <div className="py-2 px-3 rounded-xl bg-[var(--md-sys-color-surface-container)] text-[11px] text-[var(--md-sys-color-on-surface-variant)] text-center">
            No coworkers active right now.
          </div>
        )}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-3 sm:gap-6 mt-3 sm:mt-6">
        <div className="flex flex-col gap-2.5 sm:gap-4">
          <div className="flex items-center justify-between">
            <h3 className="text-xs sm:text-base font-bold flex items-center gap-2">
              <History className="w-3.5 h-3.5 sm:w-4 sm:h-4 text-[var(--md-sys-color-primary)]" />
              Recent Shift Activity
            </h3>
            <Link
              href="/candidate/attendance"
              className="text-[11px] sm:text-xs font-semibold text-[var(--md-sys-color-primary)] hover:underline flex items-center gap-1 cursor-pointer active:scale-95"
            >
              Full History <ArrowRight className="w-3 h-3 sm:w-3.5 sm:h-3.5" />
            </Link>
          </div>
        ...
        <div className="hidden lg:flex flex-col gap-4">
          ...
          <Card variant="outlined" className="flex flex-col border border-[var(--md-sys-color-outline-variant)] h-full p-4">
            {allActiveSessions && allActiveSessions.length > 0 ? (
              <div className="flex flex-col divide-y divide-[var(--md-sys-color-outline-variant)] max-h-[400px] overflow-y-auto pr-1">
                {allActiveSessions.map((session) => {
                  const profileObj = Array.isArray(session.profiles) ? session.profiles[0] : session.profiles
                  const isMe = session.user_id === userId
                  const isOnBreak = !!session.break_start_time
